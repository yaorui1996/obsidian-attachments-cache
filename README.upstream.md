...
> **This is a fork of [luisbs/obsidian-attachments-cache](https://github.com/luisbs/obsidian-attachments-cache)** with additional features. The original plugin stores remote attachments locally inside the vault. See [Fork features](#fork-features) for what this fork adds, and [Changelog](./CHANGELOG.md) for the upstream release history.
...
Features added on top of the upstream plugin. Each feature documents what it does, which trigger it uses, and how to configure it.

### Live Preview / Source mode caching `handle_onedit` (2026-09-05)

The upstream plugin only auto-caches in **Reading View** (`registerMarkdownPostProcessor` does not run for editor rendering in Live Preview or Source mode). This fork adds a text-level trigger that works in every editor mode:

- **When a note is opened/switched** (`active-leaf-change`): the editor content is scanned and all matching remote attachments are downloaded immediately.
- **While editing** (`editor-change`, debounced 1s): new links typed or inserted are cached shortly after.

Key behaviors:

- Downloads attachments without modifying the note text — the same `cache` semantics as Reading View. Text rewriting (`![[local]]`) stays under the [CacheRule `archive` setting](./docs/settings.md#cacherule-update-attachment-link) and is only performed by Reading View or the manual commands.
- Repeated scans are cheap: resolved paths are memoized and already-downloaded files are skipped.
- Configure it in **Settings → Triggers → Cache the attachments when editing notes** (see [docs/settings.md → Triggers](./docs/settings.md#triggers-settings)); toggle it off to restore upstream behavior.
- Note: like upstream, this has **no extension whitelist** — any `http(s)` URL whose path ends with an extension-like suffix and matches the CacheRule remotes is cached. Prefer restricting the [CacheRule remotes list](./docs/settings.md#cacherule-remotes) if you only want images.
...
- [x] Caching in **Live Preview / Source mode** when notes are opened or edited ([`handle_onedit`](#live-preview--source-mode-caching-handle_onedit-2026-09-05)).
...
> _Note:_ This list documents formats that Obsidian can **render** once cached — it is not a whitelist enforced by the caching logic. Any URL passing the [CacheRule remotes](./docs/settings.md#cacherule-remotes) check with an extension-like URL path is downloaded.
